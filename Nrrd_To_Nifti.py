...
for file in files:
  ...
  #Change Orientation from PIR to RAS. Steps:  PIR -> RIP -> RPS -> RAS
  ...
  #For testing purposes, save the HeaderFile of the Nifti
  image = nibabel.load(os.path.join(path, os.path.basename(file).split(".")[0]+ '.nii.gz'))
  with open(os.path.join(path, os.path.basename(file).split(".")[0] + '_Nifti_header.txt'),'w') as d:
    print(image.header, file=d)
